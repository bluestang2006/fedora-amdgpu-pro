...
# build the package
mock -r /etc/mock/fedora-37-$BUILDARCH.cfg --enable-network --rebuild *.src.rpm
...
if [[ "$BUILDARCH" == "i386" ]]; then
	sudo mv /var/lib/mock/fedora-37-i686/result/*.rpm packages/
else
	sudo mv /var/lib/mock/fedora-37-$BUILDARCH/result/*.rpm packages/
fi
...
# cleanup
mock -r /etc/mock/fedora-37-x86_64.cfg --scrub=all
mock -r /etc/mock/fedora-37-i386.cfg --scrub=all
